...
func TestReMux_Regex(t *testing.T) {
	...
	if err := mux.RegisterRegex(GET, getRegex, http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		writer.Write([]byte(GET))
	})); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{name: "GET", args: args{method: GET, path: "/resources/1/subresources/2"}, want: []byte(GET)},
		{name: "POST", args: args{method: POST, path: "/resources/1/subresources/2"}, want: []byte(POST)},
		// TODO: write for other methods
	}
